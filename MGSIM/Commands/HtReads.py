...
Description:
  ...
  * other columns are allowed
  ...
  Output
  ------
  * >=1 fastq file of reads 
    * read naming: <taxon>|<contig>|<frag_start>|<frag_end>-<barcode>-<artID>/<pair>
      * taxon = taxon name
      * contig = contig name
      * frag_start = simulated fragment start position on contig
      * frag_start = simulated fragment end position on contig
      * barcode = barcode ID
      * artID = generated by art_illumina
      * pair = read pair ID
"""
...
def main(args):
    ...
    * for each barcode batch (eg., 1000 barcodes); (in parallel)
    ...
    if args['--debug']:
        fq_files = map(func, barcodes)
    else:
        Pool = mp.Pool(int(args['-n']))
        fq_files = Pool.map(func, barcodes)
